...
typealias StrokeColorSelectionClosure = (UIColor) -> Void
typealias StrokeSelectionClosure = (CGFloat) -> Void

let strokeColorCellIdentifier = "ColorCollectionViewCell"
let strokeClearColorCellIdentifier = "ClearColorCVCell"


class StrokeColorEditSectionVC: UIViewController {
    ...
    var didSelectColor: StrokeColorSelectionClosure?
    var didSelectStroke: StrokeSelectionClosure?
    ...
    @IBAction func sliderValueChanged(_ slider: UISlider) {
        strokeLabel.text = String(Int(slider.value))
        let strokeValue = strokeWidthValue(from: slider.value)
        didSelectStroke?(strokeValue)
    }
    
    
    func strokeWidthValue(from sliderValue: Float) -> CGFloat {
        // Clamp value between 0 and 100
//        let clamped = min(max(sliderValue, 0), 100)
        // Map 0–100 to 0 to -10
        return CGFloat(-sliderValue / 20.0)
    }

}


extension StrokeColorEditSectionVC: UICollectionViewDelegate {
...
extension StrokeColorEditSectionVC: UICollectionViewDataSource {
...
extension StrokeColorEditSectionVC: UICollectionViewDelegateFlowLayout {
    func collectionView(
      _ collectionView: UICollectionView,
